import { render, screen, fireEvent } from '@testing-library/react';
import { describe, it, expect, vi } from 'vitest';
import { PostCard } from '@/features/feed/components/PostCard/PostCard';
import { Post, PostType, PostStatus } from '@/features/feed/types/post';
import { Timestamp } from 'firebase/firestore';

// PostCard uses next/image (requires a server) and useRouter — mock both
vi.mock('next/navigation', () => ({
  useRouter: vi.fn(() => ({ push: vi.fn() })),
}));

vi.mock('next/image', () => ({
  default: ({ src, alt }: { src: string; alt: string }) => <img src={src} alt={alt} />,
}));

const mockPost: Post = {
    id: '123',
    authorId: 'user1',
    author: { uid: 'user1', displayName: 'Test Author', email: 'test@test.com', photoURL: null, bio: '', createdAt: {} as any, updatedAt: {} as any },
    type: PostType.FAILURE,
    status: PostStatus.PUBLISHED,
    title: 'My Dead Plant',
    content: 'It died so fast...',
    images: ['https://example.com/plant.jpg'],
    environment: {
        userId: 'user1',
        residenceType: 'APARTMENT',
        lightDirection: 'SOUTH',
        experienceLevel: 'BEGINNER',
        createdAt: '2023-01-01',
        updatedAt: '2023-01-01',
        snapshotAt: {} as Timestamp,
    },
    plant: { name: 'Monstera', imageUrls: [] },
    views: 10,
    likes: 5,
    commentsCount: 2,
    createdAt: {} as Timestamp,
    updatedAt: {} as Timestamp,
};

describe('PostCard', () => {
    it('renders post information correctly', () => {
        render(<PostCard post={mockPost} />);

        // Plant name is rendered in the heading
        expect(screen.getByText(/Monstera/i)).toBeDefined();
        // Content is rendered
        expect(screen.getByText('It died so fast...')).toBeDefined();
        // Author is rendered
        expect(screen.getByText('By Test Author')).toBeDefined();
        // Likes and comments are rendered (PostCard shows 👍 and 💬, not 👁️)
        expect(screen.getByText(/👍/)).toBeDefined();
        expect(screen.getByText(/💬/)).toBeDefined();
    });

    it('calls onClick handler when clicked', () => {
        const handleClick = vi.fn();
        render(<PostCard post={mockPost} onClick={handleClick} />);

        // Click the card wrapper
        fireEvent.click(screen.getByText(/Monstera/i).closest('div')!);
        expect(handleClick).toHaveBeenCalledTimes(1);
    });
});
